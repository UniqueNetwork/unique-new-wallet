...
export class PolkadotjsExtensionPage extends BasePage {
  ...
  getAccountFullAddressLocatorByName(name: string): Locator {
    return this.page
      .locator(`//span[@title='${name}']/../..`)
      .locator('//div[@class="fullAddress"]');
  }
  ...
  async connectAccountByExtension(
    ...
    await this.nextStepButton.click();
    await expect(
      this.getAccountFullAddressLocatorByName(accountName),
      `account with address ${accountAddress} not loaded`,
    ).toHaveText(accountAddress);
  }

  static async connectAccountByExtension(
    page: Page,
    mnemonic: string,
    password: string,
    accountName: string,
    ...
    await polkdotExtensionPage.connectAccountByExtension(
      mnemonic,
      password,
      accountName,
      accountAddress,
