...
export async function generateAccount(balance = 0) {
  const testUser = await getRequest(`account/generate`, 'POST', {
    pairType: 'sr25519',
    meta: {},
  });
...
export async function transferBalance(
  ...
  const payload = await getRequest(`balance/transfer`, 'POST', {
    address: addressFrom,
...
export async function createCollection(
  ...
  const payload = await getRequest(`collection`, 'POST', {
...
export async function createToken(
  tokenParam: { address: string; collectionId: number },
  seed: string,
) {
  const payload = await getRequest('token', 'POST', {
    address: tokenParam.address,
    collectionId: tokenParam.collectionId,
    owner: tokenParam.address,
    constData: {
      ipfsJson:
        '{"ipfs":"QmVrF9EJJJYeUVkqrtELe5dujMQ9KWyaMeWxp8Qj3zQLyV","type":"image"}',
      gender: 'Male',
...
export async function transferToken(params: {
  ...
  return await getRequest('token/transfer', 'PATCH', params);
}

export async function transferCollection(params: {
  collectionId: number;
  from: string;
  to: string;
}) {
  return await getRequest('collection/transfer', 'PATCH', params);
}

async function signExtrinsic(payload: any, seed: string) {
  return await getRequest(`extrinsic/sign`, 'POST', payload, {
...
async function submitExtrinsic(signerPayloadJSON: any, signature: any) {
  return await getRequest(`extrinsic/submit`, 'POST', {
    signerPayloadJSON,
    signature,
  });
}
...
async function waitCompletedStatusExtrinsic(hash: any, n: number) {
  const response = await getRequest(`extrinsic/status?hash=${hash}`);
  if (response.isCompleted == true) {
    return;
  }
  if (n <= 1) {
    throw new Error('Transfer balance error');
  }
  await sleep(1000);
